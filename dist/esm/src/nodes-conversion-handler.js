    ...
    if (rows.length > 0) {
        for (var i = 0; i < rows.length; i++) {
            var row = {
                "nodeType": "table-row",
                "data": {},
                "content": Array.from(rows[i].childNodes).map(function (node) { return nodesToContentful(node, parentMarks); }).filter(function (el) { return el !== false; })
            };
            array.push(row);
        }
    }
        ...
        Array.from(nodes).forEach(function (node) {
            var result = nodesToContentful(node, parentMarks);
            if (result !== false) {
...
export var nodesToContentful = function (node, marks) {
    var nodeNameMap = {
        'H1': 'heading-1',
        'H2': 'heading-2',
        'H3': 'heading-3',
...
export var nodesToHtml = function (node) {
    ...
    var sub = node.content.map(nodesToHtml).join('');
    var tagNameMap = {
        'paragraph': 'p',
        'heading-1': 'h1',
        'heading-2': 'h2',
